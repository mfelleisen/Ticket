...
(module+ test
  ...
  (define okay2 (connection Orlando |San Diego| blue 5))
  ...
  (check-equal? (send strat-tri choose-action pstate2) okay)
  (check-equal? (send strat-tri choose-action (like-pstate2 'green 3)) MORE)
  (check-equal? (send strat-tri more-cards 'green 'red) (void))
  (check-equal? (send strat-rec choose-action (like-pstate2 'green 2)) okay2)
